...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    if(queryString !== "") {
      ...
      if(urlParams.has("code")) {
        this.setState({ socket: socketIOClient("http://localhost:8000") }, () => {
          this.state.socket.on(CONNECT, this.socketEstablished(urlParams.get("code")))
        });
      }
    }
  }
  ...
  render() {
    return (
      <div>
        <a href={`http://localhost:8000/login?userID=${this.state.userID}`}>Try sign in</a>
      </div>
    );
  }
}
